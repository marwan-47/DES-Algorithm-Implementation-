#include <stdio.h>
#include <stdint.h>
#include <string.h>

const int IP[64] = {
    58, 50, 42, 34, 26, 18, 10,  2,
    60, 52, 44, 36, 28, 20, 12,  4,
    62, 54, 46, 38, 30, 22, 14,  6,
    64, 56, 48, 40, 32, 24, 16,  8,
    57, 49, 41, 33, 25, 17,  9,  1,
    59, 51, 43, 35, 27, 19, 11,  3,
    61, 53, 45, 37, 29, 21, 13,  5,
    63, 55, 47, 39, 31, 23, 15,  7
};

const int FP[64] = {
    40,  8, 48, 16, 56, 24, 64, 32,
    39,  7, 47, 15, 55, 23, 63, 31,
    38,  6, 46, 14, 54, 22, 62, 30,
    37,  5, 45, 13, 53, 21, 61, 29,
    36,  4, 44, 12, 52, 20, 60, 28,
    35,  3, 43, 11, 51, 19, 59, 27,
    34,  2, 42, 10, 50, 18, 58, 26,
    33,  1, 41,  9, 49, 17, 57, 25
};

const int E[48] = {
    32,  1,  2,  3,  4,  5,
     4,  5,  6,  7,  8,  9,
     8,  9, 10, 11, 12, 13,
    12, 13, 14, 15, 16, 17,
    16, 17, 18, 19, 20, 21,
    20, 21, 22, 23, 24, 25,
    24, 25, 26, 27, 28, 29,
    28, 29, 30, 31, 32,  1
};

const int P[32] = {
    16,  7, 20, 21,
    29, 12, 28, 17,
     1, 15, 23, 26,
     5, 18, 31, 10,
     2,  8, 24, 14,
    32, 27,  3,  9,
    19, 13, 30,  6,
    22, 11,  4, 25
};

const int PC1[56] = {
    57, 49, 41, 33, 25, 17,  9,
     1, 58, 50, 42, 34, 26, 18,
    10,  2, 59, 51, 43, 35, 27,
    19, 11,  3, 60, 52, 44, 36, 
    63, 55, 47, 39, 31, 23, 15,
     7, 62, 54, 46, 38, 30, 22,
    14,  6, 61, 53, 45, 37, 29,
    21, 13,  5, 28, 20, 12,  4
};

const int PC2[48] = {
    14, 17, 11, 24,  1,  5,
     3, 28, 15,  6, 21, 10,
    23, 19, 12,  4, 26,  8,
    16,  7, 27, 20, 13,  2,
    41, 52, 31, 37, 47, 55,
    30, 40, 51, 45, 33, 48,
    44, 49, 39, 56, 34, 53,
    46, 42, 50, 36, 29, 32
};

const int SHIFTS[16] = {
    1, 1, 2, 2, 2, 2, 2, 2,
    1, 2, 2, 2, 2, 2, 2, 1
};

const int S1[4][16] = {
    {14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7},
    {0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8},
    {4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0},
    {15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13}
};

const int S2[4][16] = {
    {15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10},
    {3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5},
    {0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15},
    {13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9}
};

const int S3[4][16] = {
    {10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8},
    {13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1},
    {13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7},
    {1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12}
};

const int S4[4][16] = {
    {7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15},
    {13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9},
    {10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4},
    {3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14}
};

const int S5[4][16] = {
    {2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9},
    {14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6},
    {4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14},
    {11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3}
};

const int S6[4][16] = {
    {12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11},
    {10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8},
    {9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6},
    {4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13}
};

const int S7[4][16] = {
    {4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1},
    {13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6},
    {1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2},
    {6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12}
};

const int S8[4][16] = {
    {13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7},
    {1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2},
    {7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8},
    {2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11}
};

const int (*SBOXES[8])[16] = { S1, S2, S3, S4, S5, S6, S7, S8 };

uint64_t permute(uint64_t input, const int *table, int table_size, int input_size){
    uint64_t output = 0;
    for (int i = 0; i < table_size; i++) {
        int src_bit_index = input_size - table[i];  
        uint64_t bit = (input >> src_bit_index) & 1ULL;
        output = (output << 1) | bit;
    }
    return output;
}

uint64_t initial_permutation(uint64_t input){
    return permute(input, IP, 64, 64);
}

uint64_t final_permutation(uint64_t input){
    return permute(input, FP, 64, 64);
}

uint64_t expansion(uint32_t right_half){
    return permute((uint64_t)right_half, E, 48, 32);
}

uint32_t permutation_pbox(uint32_t input){
    return (uint32_t)permute((uint64_t)input, P, 32, 32);
}

uint64_t key_permutation_pc1(uint64_t key64){
    return permute(key64, PC1, 56, 64);
}

uint64_t key_permutation_pc2(uint64_t combined56){
    return permute(combined56, PC2, 48, 56);
}


uint32_t left_rotate28(uint32_t value, int shift){
    value &= 0x0FFFFFFF; // mask to 28 bits
    return ((value << shift) | (value >> (28 - shift))) & 0x0FFFFFFF;
}


uint32_t sbox_substitution(uint64_t input48){
    uint32_t output32 = 0;

    for (int i = 0; i < 8; i++) {
        int six_bits = (input48 >> (42 - 6*i)) & 0x3F; // extract 6 bits
        int row = ((six_bits & 0x20) >> 4) | (six_bits & 0x01);
        int col = (six_bits >> 1) & 0x0F;
        int s_val = SBOXES[i][row][col];
        output32 = (output32 << 4) | (s_val & 0x0F);
    }

    return output32;
}

uint32_t feistel(uint32_t right, uint64_t subkey){
    uint64_t expand = expansion(right);
    uint64_t xor = expand ^ subkey;
    uint32_t subBox = sbox_substitution(xor);
    return permutation_pbox(subBox);
}

void generate_subkeys(uint64_t key, uint64_t subkeys[16]){
    uint64_t pc1 = key_permutation_pc1(key);
    uint32_t right = pc1 & 0xFFFFFFF;
    uint32_t left = (pc1 >> 28) & 0x0FFFFFFF;

    for(int i = 0; i < 16; i++){
    right = left_rotate28(right,SHIFTS[i]);
    left = left_rotate28(left,SHIFTS[i]);
    uint64_t combined56 = ((uint64_t)left << 28) | right;
    subkeys[i] = key_permutation_pc2(combined56);
    }
   
}

void des_encrypt(uint64_t *block, uint64_t subkeys[16]){
    uint64_t data = *block;
    data = initial_permutation(data);

    uint32_t L = (data >> 32) & 0xFFFFFFFF;
    uint32_t R = data & 0xFFFFFFFF;

    for (int i = 0; i < 16; i++) {
        uint32_t temp = R;
        R = L ^ feistel(R, subkeys[i]);
        L = temp;
    }

    data = ((uint64_t)R << 32) | L;
    data = final_permutation(data);
    *block = data;
}

void des_decrypt(uint64_t *block, uint64_t subkeys[16]){
    uint64_t data = *block;
    data = initial_permutation(data);

    uint32_t L = (data >> 32) & 0xFFFFFFFF;
    uint32_t R = data & 0xFFFFFFFF;

    for (int i = 0; i < 16; i++) {
        uint32_t temp = R;
        R = L ^ feistel(R, subkeys[15-i]);
        L = temp;
    }

    data = ((uint64_t)R << 32) | L;
    data = final_permutation(data);
    *block = data;
}


int main(int argc, char **argv)
{
    if (argc != 5) {
        printf("Usage:\n");
        printf("  %s e key plaintext ciphertext\n", argv[0]);
        printf("  %s d key ciphertext plaintext\n", argv[0]);
        return 1;
    }

    char mode = argv[1][0];         
    const char *key_filename = argv[2];
    const char *input_filename = argv[3];
    const char *output_filename = argv[4];

    FILE *fkey = fopen(key_filename, "rb");
    FILE *fin  = fopen(input_filename, "rb");
    FILE *fout = fopen(output_filename, "wb");

    if (!fkey || !fin || !fout) {
        printf("Error: could not open one of the files.\n");
        if (fkey) fclose(fkey);
        if (fin) fclose(fin);
        if (fout) fclose(fout);
        return 1;
    }

    uint64_t key = 0;
    fread(&key, sizeof(uint64_t), 1, fkey);
    fclose(fkey);

    uint64_t subkeys[16];
    generate_subkeys(key, subkeys);

    uint64_t block;

    if (mode == 'e') {
        while (fread(&block, sizeof(uint64_t), 1, fin) == 1) {
            des_encrypt(&block, subkeys);
            fwrite(&block, sizeof(uint64_t), 1, fout);
        }
    }
    else if (mode == 'd') {
        while (fread(&block, sizeof(uint64_t), 1, fin) == 1) {
            des_decrypt(&block, subkeys);
            fwrite(&block, sizeof(uint64_t), 1, fout);
        }
    }
    else {
        printf("Invalid mode: use 'e' for encrypt or 'd' for decrypt.\n");
    }

    fclose(fin);
    fclose(fout);

    return 0;
}
